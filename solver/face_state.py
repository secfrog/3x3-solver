...
with open('assets/config/CubeState.txt', 'w') as cube:
    cube.write('')
...
def display_face_state(face_state):
    ...
    with open('assets/config/CubeState.txt', 'a') as cube:
        cube.write(f'{face_state}\n')

    # Show the cube state in a new window
    color_list = ['yellow', 'blue', 'orange', 'white', 'green', "red"]
    with open('assets/config/c.txt', 'r') as cr:
        c = cr.read()
        cv2.imwrite(f'assets/images/{c}face_State.jpg', face_display)
        print('face representation done')
